...
## 2026-08-28 [decision] gh版本过旧时主动升级，而非仅兼容降级

- v1.7.0 检测到 gh < 2.25 只做兼容降级，用户指出应该直接装最新版
- v1.8.0 启动检查改为：检测到旧版 gh 先尝试自动升级，升级失败才降级兼容
- gh-cli-setup.md 补充无 sudo/沙箱环境的二进制安装流程（装到 ~/.local/bin）
- 补充 GitHub release 下载镜像加速（ghfast.top），沙箱直连仅 13KB/s，镜像可达 900KB/s
- 明确禁止直接 `apt install gh`（发行版仓库版本极旧），必须用官方 apt 源或二进制安装
- 新增"升级"章节，定义有sudo/无sudo/升级失败三级策略
